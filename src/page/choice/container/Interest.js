...
export class JobBasedOnInterest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      backend: null,
  ...
  async componentDidMount() {
    this.setState({
      backend: await getAsync(`/job-for-you/campus`, true)
    });
  ...
  render() {
    ...
    return (
      (this.state.backend && this.state.backend.status && this.state.backend.status.code === 200) ?
        <MDBRow left className={`justify-content-center ${this.props.background}`}
                style={{
