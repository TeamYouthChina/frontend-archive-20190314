...
export class QuestionCard extends React.Component {
  ...
  async componentDidMount() {
    ...
    if (this.props.questionId) {
      ...
      try {
        result = await getAsync(`/questions/${this.props.params.match.questionId}`)
      } catch (e) {
      ...
      try {
        if(user.answers.length !== 0){
          commonLists = await getAsync(`/answers/${user.answers[0].id}/comments`,!data.env)
          commonLists = commonLists.content.comments
        }
      } catch (e) {
        // alert(e)
