...
export class QuestionDes extends React.Component {
  constructor(props) {
    ...
    * */
    this.state = {
      backend: null,
      editorState: BraftEditor.createEditorState(this.props.content.description),
      // testNum:true,
      selectType: 1,
      hiddenAns: false,
      showText: false,
        ...
        <div style={{width: '100%'}}>
          {this.props.loading ? (
            ...
            <MDBCol size="9">
              ...
              <p 
                style={{color: '#62686C', fontSize: '14px', ...basicFont}} 
                dangerouslySetInnerHTML={{__html: this.state.editorState.toHTML(this.props.content.description)}}>
              </p>
            </MDBCol>
            <MDBCol size="3">
              <MDBRow>
          ...
          <MDBRow>
            <MDBBtn flat style={{padding: '5px 10px', marginLeft: '5px'}}>
              <MDBIcon style={{marginRight: '5px'}} icon="heart"/>{this.text.toFocus}
            </MDBBtn>
            <MDBBtn onClick={this.handleClick} flat style={{padding: '5px 10px',}}>
              <MDBIcon style={{marginRight: '5px'}}  icon="edit"/>
